...
<main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        @if(Auth::user()->hasPermission(App\Enums\PermissionEnum::from('manage_photos'), $event))
            <a href="{{ route('photos.pending', $event) }}" class="group flex items-center gap-4 p-4 bg-white border border-slate-200 rounded-xl shadow-sm hover:border-indigo-300 hover:shadow-md transition">
        ...
        @if(Auth::user()->hasPermission(App\Enums\PermissionEnum::from('view_guests'), $event))
            <a href="{{ route('guests.index', $event) }}" class="group flex items-center gap-4 p-4 bg-white border border-slate-200 rounded-xl shadow-sm hover:border-pink-300 hover:shadow-md transition">
